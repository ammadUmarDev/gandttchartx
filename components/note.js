...
const Note = ({ details }) => {
  ...
  let note = useSelector((redux) =>
    redux.notes.find((note) => note.id == details.id)
  );
  ...
  const [underline, setUnderline] = useState(
    note.underline ? note.underline : false
  );
  ...
  useEffect(() => {
    document.getElementById(`${note.id}`).innerHTML = note.text
      ? note.text
      : '';
  }, []);
  ...
  const dragStart = (e) => {
    ...
    if (
      ...
      30 &&
      e.clientY >=
      e.currentTarget.getBoundingClientRect().top + e.target.offsetHeight - 30
    ) {
      setDragFlag(false);
    } else {
      setDragFlag(true);
    ...
    >
      ...
      >
          ...
          <div className='toolbar'>
            <div className='toolbar-left'>
              ...
              <i
                ...
                onClick={() => setItalic((state) => !state)}
                style={{ fontWeight: italic ? 'bold' : 'normal' }}
                type='button'
                aria-hidden='true'
              />
              <i
                className='fa fa-underline'
                onClick={() => setUnderline((state) => !state)}
                style={{ fontWeight: underline ? 'bold' : 'normal' }}
                type='button'
                aria-hidden='true'
              />
              <i
                onClick={(e) => setOrdered(true)}
                className='fa fa-list-ol'
                aria-hidden='true'
              />
              <i
                onClick={(e) => setUnordered(true)}
              ...
              <select
                ...
                id='font-manager'
              >
                <option value='volvo' style={{ fontSize: 10 }}>
                  Font Size
                </option>
                <option value={8} style={{ fontSize: 8 }}>
                  1 (8pt)
                </option>
                <option value={10} style={{ fontSize: 10 }}>
                  2 (10pt)
                </option>
                <option value={12} style={{ fontSize: 12 }}>
                  3 (12pt)
                </option>
                <option value={14} style={{ fontSize: 14 }}>
                  4 (14pt)
                </option>
                <option value={18} style={{ fontSize: 18 }}>
                  5 (18pt)
                </option>
                <option value={24} style={{ fontSize: 24 }}>
                  6 (24pt)
                </option>
              </select>
              <i
            ...
            {colors.map((val, index) => (
              <div
                ...
                style={{
                  ...
                  width: 40,
                  backgroundColor: val,
                  border: color == val ? '3px solid black' : '0px solid black',
                }}
              />
            ))}
          </div>
        ...
        <div
          ...
          className={`textArea ${note.isActive ? 'nothing' : 'handle'}`}
          id={`${note.id}`}
          onInput={(e) => {
            obj.current = e;
          ...
          style={{
            width: width,
            height: height,
            backgroundImage: note.noteColor,
            color: color,
